...
class Price(Base):
    ...
    refining_margin  = db.Column(db.Float(10, 2),  nullable=True)
    ...
    def __init__(self, date, dubai, wti, complex_margin, refining_margin,
                 naphtha, ethylene, propylene, butadiene, benzene, toluene, px,
                 sm, pta, meg, ldpe, lldpe, hdpe, pp, cotton, rubber):
        super().__init__()
        self.date             = date
        self.dubai            = safe_execute(float, dubai)
        self.wti              = safe_execute(float, wti)
        self.refining_margin  = safe_execute(float, refining_margin)
